...
1. **Logging** → Records method name, duration, and errors to structured logs
2. **Tracing** → Adds OpenTelemetry spans (when enabled)
3. **Metrics** → Increments counters and records latency histograms
...
**Architecture:** The Manager acts as a proxy between proplets and an external FL Coordinator service. The coordinator handles aggregation algorithms (FedAvg, etc.) while the Manager handles task distribution and result collection.

```
Proplet → MQTT → Manager → HTTP → FL Coordinator
                    ↑
               Aggregates
               model updates
```
...
```bash
export COORDINATOR_URL=http://fl-coordinator:8080
```

The Manager then proxies these FL endpoints to the coordinator:

| Endpoint | Purpose |
| -------- | ------- |
| `POST /fl/experiments` | Configure FL experiment (rounds, aggregation method) |
| `POST /fl/task` | Request training task for a proplet |
| `POST /fl/update` | Submit model update from proplet |
| `GET /fl/rounds/{id}` | Check round completion status |

##### When FL is Disabled

When `COORDINATOR_URL` is not set:
- The Manager logs a warning at startup: `"COORDINATOR_URL not configured - FL features will not be available"`
- All FL endpoints return an error: `"COORDINATOR_URL must be configured"`
...
##### FL Coordinator Requirements

The external coordinator must implement these HTTP endpoints:

| Method | Path | Request | Response |
| ------ | ---- | ------- | -------- |
| `POST` | `/experiments` | Experiment config JSON | Experiment ID |
| `POST` | `/task` | `{round_id, proplet_id}` | Training task with model weights |
| `POST` | `/update` | Model gradients (CBOR) | Acknowledgment |
| `GET` | `/rounds/{id}` | — | Round status |

For a reference implementation, see the [Federated Learning](/docs/federated-learning) documentation.
