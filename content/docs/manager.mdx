...
The Manager acts as a proxy between proplets and an external FL Coordinator service. The coordinator handles aggregation algorithms (FedAvg, etc.) while the Manager handles task distribution and result collection. For a complete overview of the architecture and training round lifecycle, see the [Federated Learning](/docs/federated-learning) documentation.
...
The Manager then proxies FL endpoints to the coordinator. For the complete endpoint reference, see the [Federated Learning API](/docs/api/federated-ml).
