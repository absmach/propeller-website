...
For a hands-on tutorial with commands and expected outputs, see the [Getting Started](/docs/getting-started) guide. For the complete task data model including all fields and states, see the [Reference](/docs/reference#task) documentation.
...
For detailed proplet configuration and capabilities, see the [Proplet](/docs/proplet) documentation.
...
func (p *Proplet) SetAlive() {
    if len(p.AliveHistory) > 0 {
        p.Alive = time.Since(p.AliveHistory[len(p.AliveHistory)-1]) <= aliveThreshold
    }
}
```
...
**Dead proplets are excluded from scheduling** — The round-robin scheduler skips any proplet where `Alive == false`. If ALL proplets are dead, task start commands fail with `ErrDeadProplets`.
...
// From manager/service.go
if p.TaskCount > 0 {
    return fmt.Errorf("proplet %s has %d active tasks", propletID, p.TaskCount)
}
```
...
**Job Start** (`POST /jobs/{id}/start`) — The Manager's `StartJob` method dispatches tasks based on the stored execution mode:

| Mode | Manager Behavior |
| ---- | ---------------- |
| `parallel` | Calls `StartTask` for ALL tasks simultaneously; each gets its own proplet |
| `sequential` | Calls `StartTask` for the first task only; subsequent tasks start when previous completes |
| `configurable` | Reads mode from `JOB_EXECUTION_MODE` environment variable at runtime |
...
**Sequential chaining** — When a task completes, the Manager checks if it belongs to a job. If so, and the job is sequential, it automatically starts the next task in order.
...
if t.State == task.Failed {
    svc.logger.Info("task failed, stopping remaining job tasks", "job_id", t.JobID)
...
For detailed workflow patterns, conditional execution, and examples, see [Task Scheduling](/docs/task-scheduling).
...
func (wc *WorkflowCoordinator) EvaluateConditionalExecution(ctx context.Context, t task.Task, parentStates map[string]task.State) bool {
    ...
    case task.RunIfFailure:
        // At least one parent must be Failed
        for _, depID := range t.DependsOn {
...
```
1. POST /workflows          → Manager validates DAG and creates all tasks
2. POST /tasks/{root}/start → User starts the root task(s)
3. Root task completes      → Manager calls OnTaskCompletion
4. Coordinator evaluates    → CheckAndStartReadyTasks finds eligible tasks
5. Dependent tasks start    → Repeat until all tasks terminal
```

The workflow coordinator runs automatically—you only need to start the root task(s).
...
When a task starts without a specified `proplet_id`, the Manager's scheduler selects one:

```go
// From pkg/scheduler/roundrobin.go
...
For recurring tasks, the Manager runs a cron scheduler that checks for due tasks every 60 seconds:
...
**Manager's cron loop:**
1. Every 60 seconds, scan all tasks with `is_recurring = true`
2. Parse the `schedule` using the specified `timezone`
3. If the task is due, call `StartTask`
4. Track last execution to prevent duplicate runs

### Priority Ordering

Tasks have an optional `priority` field (0–100, higher = more important). The Manager uses priority when:
- Multiple tasks are due simultaneously in cron
- Sequential job execution determines next task

Priority affects dispatch ORDER, not proplet selection. A priority=100 task is started before a priority=0 task, but both go through round-robin for proplet assignment.

| Mechanism | What It Controls |
| --------- | ---------------- |
| **Round Robin** | WHICH proplet receives the task |
| **Priority** | WHEN the task is dispatched (relative to other pending tasks) |
