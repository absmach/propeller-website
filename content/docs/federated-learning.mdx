...
1. **Modify the training loop** in `fl-client.go` (lines 180-240):
...
### Example: Implementing FedProx

FedProx adds a proximal term to limit how far local updates can deviate from the global model:

**1. WASM Client modification** (`fl-client.go`):
...
**2. Aggregator** - No changes needed (FedProx uses standard FedAvg aggregation)

### Example: Implementing Byzantine-Robust Aggregation

For environments where some proplets may submit malicious updates:

**Aggregator modification** (`aggregator/main.go`):
