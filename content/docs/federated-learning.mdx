...
- [FedProx Algorithm](/docs/examples/fedprox) - Modify the WASM client to handle non-IID data with proximal regularization
- [Byzantine-Robust Aggregation](/docs/examples/byzantine-robust) - Replace FedAvg with median-based aggregation for untrusted environments
...
|----------|-------------|---------|
| `COORDINATOR_URL` | URL of FL Coordinator service. If not set, FL features are disabled and a warning is logged. | `""` (FL disabled) |
...
| `COORDINATOR_URL` | URL of FL Coordinator | `http://coordinator-http:8080` |
