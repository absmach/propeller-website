...
:::warning Using SuperMQ CLIENT_IDs
...
For detailed setup instructions and hands-on examples, see the [FL Demo](https://github.com/absmach/propeller/tree/main/examples/fl-demo).
...
```bash
# Login to GHCR (requires PAT with write:packages permission)
echo $GITHUB_TOKEN | docker login ghcr.io -u YOUR_GITHUB_USERNAME --password-stdin

# Push using ORAS (from repository root)
docker run --rm \
...
```bash
# Build images (Manager and proplet must be built from source)
docker compose -f docker/compose.yaml -f examples/fl-demo/compose.yaml \
  --env-file docker/.env build manager proplet proplet-2 proplet-3 coordinator-http
...
This creates domain, channel, and clients (manager, proplet-1, proplet-2, proplet-3, fl-coordinator, proxy) and updates `docker/.env`.

#### Step 5: Recreate Services

After provisioning, recreate services to pick up new credentials:

```bash
docker compose -f docker/compose.yaml -f examples/fl-demo/compose.yaml \
  --env-file docker/.env up -d --force-recreate manager coordinator-http proplet proplet-2 proplet-3 proxy
```
...
# Check aggregated model (after round completes)
