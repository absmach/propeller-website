...
Propeller implements **Federated Averaging (FedAvg)** for model aggregation. FedAvg computes a weighted average of model updates based on the number of training samples each proplet used.

### FedAvg Formula

For n participating proplets with updates $u_1, u_2, ..., u_n$ and sample counts $s_1, s_2, ..., s_n$:

$$
w_{aggregated} = \frac{\sum_{i=1}^{n} s_i \cdot w_i}{\sum_{i=1}^{n} s_i}
$$

$$
b_{aggregated} = \frac{\sum_{i=1}^{n} s_i \cdot b_i}{\sum_{i=1}^{n} s_i}
$$
...
$$
w_j = w_j - \alpha \cdot (p - y) \cdot x_j
$$
...
Where $\alpha$ is the learning rate, $p$ is the sigmoid prediction, and $y$ is the true label.
