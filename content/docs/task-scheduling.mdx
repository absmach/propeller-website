...
When you need to run multiple tasks that depend on each other—like "download data, then process it, then generate a report"—you need a way to ensure they run in the right order. Propeller handles this automatically using a concept called a DAG.

### What is a DAG?

A [DAG (Directed Acyclic Graph)](https://www.datacamp.com/blog/what-is-a-dag) is simply a way to describe "what depends on what." Think of it like a recipe:

1. You can't bake a cake until you've mixed the batter
2. You can't mix the batter until you've gathered the ingredients
3. But while the cake bakes, you can prepare the frosting (these are independent)

In Propeller terms:

- **Tasks** are the steps (like "gather ingredients" or "mix batter")
- **Dependencies** are the arrows showing which tasks must finish before others can start
- **Acyclic** means the workflow only moves forward—no loops or going back

The key benefit: tasks that don't depend on each other can run at the same time (in parallel), making your workflow faster.

| Benefit | What it means |
|---------|---------------|
| **Correct order** | Tasks wait for their dependencies to finish before starting |
| **Faster execution** | Independent tasks run in parallel instead of waiting in line |
| **Easy debugging** | When something fails, you can see exactly which tasks were affected |
| **Clear visualization** | The graph shows your entire workflow at a glance |
