...
Task orchestration in Propeller uses [Directed Acyclic Graphs (DAGs)](https://www.datacamp.com/blog/what-is-a-dag) to model dependencies between tasks. A DAG ensures tasks execute in the correct order—parent tasks complete before their dependents start—while allowing independent tasks to run in parallel. This model enables complex multi-step workflows like ETL pipelines, ML training jobs, and data processing chains.
