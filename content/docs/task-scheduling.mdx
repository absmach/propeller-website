...
**Execution flow (image processing pipeline):**
1. **Ingest** task runs first—loads the source image
2. **Fan-out**: Ingest triggers three parallel tasks:
   - **Resize** — scales the image
   - **Watermark** — adds branding overlay
   - **Compress** — reduces file size
3. **Fan-in**: **Merge & Upload** waits for all three to complete, then combines results and uploads

**Fan-out (one-to-many):**
- A single parent task triggers multiple child tasks
- All children start simultaneously when the parent completes
- Maximizes parallelism for independent work
