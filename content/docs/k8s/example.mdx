...
For a complete reference of the operator's architecture, Custom Resource Definitions, configuration options, and MQTT communication patterns, see the [Propeller Kubernetes Operator documentation](/docs/k8s/operator).
...
All five CRDs are now registered with the Kubernetes API server. See the [CRD reference](/docs/k8s/operator#custom-resource-definitions) for details on each resource type.
...
The table below lists common fields. For the complete specification, see the [features documentation](/docs/k8s/features#proplet-spec-reference).
...
Create a Task that targets the docker proplet. This uses the addition WASM module (the same binary used in the [addition example](/docs/examples/addition))—a minimal module that exports a `main` function taking two integers and returning their sum. See the [Task spec reference](/docs/k8s/features#task-spec-reference) for all available fields.
...
The table below lists common fields. For the complete specification, see the [features documentation](/docs/k8s/features#task-spec-reference).
