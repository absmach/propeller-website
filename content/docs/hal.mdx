...
### First boot hangs or takes too long

First boot compiles Rust projects from source, which can take 15+ minutes on slower systems. Monitor progress:

```bash
# In the VM console, check cloud-init status
...
Check if binaries were installed correctly:

```bash
ls -la /usr/local/bin/{wasmtime,attestation-agent,coco_keyprovider,proplet}
```

If any are missing, check cloud-init logs for compilation errors:

```bash
sudo cat /var/log/cloud-init-output.log | grep -A 20 "ERROR"
```

### tdx_guest module not loading

On first boot, the TDX module may not load until the extra kernel modules are installed:

```bash
# Check if module is available
lsmod | grep tdx

# Manually load if available
...
If the module isn't available, reboot the VM after cloud-init completes.

### MQTT connection fails

Verify the MQTT broker is reachable from inside the VM:

```bash
# Test connectivity (QEMU NAT maps 10.0.2.2 to host)
nc -zv 10.0.2.2 1883

# Check proplet configuration
cat /etc/default/proplet | grep MQTT
```

If using a remote broker, ensure the `PROPLET_MQTT_ADDRESS` was set correctly before running `./qemu.sh build`.

### Port forwarding not working

QEMU must be running with the correct hostfwd options. The script sets up:

```
hostfwd=tcp::2222-:22
hostfwd=tcp::50010-:50010
hostfwd=tcp::50011-:50011
```

If these ports are in use on the host, the VM may fail to start. Free the ports or modify the script.

## Next Steps

After the HAL VM is running:

| Goal | Documentation |
|------|---------------|
| Deploy encrypted WASM workloads | [TEE](./tee) |
| Understand attestation flow | [TEE - Understand the flow](./tee#understand-the-flow) |
| Configure the Key Broker Service | [TEE - Set up Trustee](./tee#set-up-trustee) |
