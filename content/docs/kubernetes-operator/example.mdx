...
For a complete reference of the operator's architecture, Custom Resource Definitions, configuration options, and MQTT communication patterns, see the [Propeller Kubernetes Operator documentation](/docs/kubernetes-operator/kubernetes-operator).
...
All five CRDs are now registered with the Kubernetes API server. See the [CRD reference](/docs/kubernetes-operator/kubernetes-operator#custom-resource-definitions) for details on each resource type.
...
The table below lists common fields. For the complete specification, see the [main documentation](/docs/kubernetes-operator/kubernetes-operator#proplet-spec-reference).
...
Create a Task that targets the docker proplet. This uses the addition WASM module (the same binary used in the [addition example](/docs/examples/addition))—a minimal module that exports a `main` function taking two integers and returning their sum. See the [Task spec reference](/docs/kubernetes-operator/kubernetes-operator#task-spec-reference) for all available fields.
...
The table below lists common fields. For the complete specification, see the [main documentation](/docs/kubernetes-operator/kubernetes-operator#task-spec-reference).
