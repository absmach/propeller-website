...
1. **Initial Request**
   The proplet sends a download request via the MQTT topic: `m/{domain_id}/c/{channel_id}/registry/proplet`
...
3. **Chunked Data Transfer**
   ...
   These chunks are sent back to the proplet via the MQTT topic: `m/{domain_id}/c/{channel_id}/registry/server`.
...
- Chunked data handling with configurable chunk size (512 KB default)
...
| Topic | Description |
| ----- | ----------- |
| [Proplet](/docs/proplet) | Configure the Proplet runtime that consumes Wasm modules |
| [Manager](/docs/manager) | Configure the Manager that orchestrates task distribution |
