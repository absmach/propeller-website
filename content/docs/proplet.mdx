...
| `PROPLET_CLIENT_ID`             | The client ID used as the proplet's identity and for MQTT authentication.                            |                        |
| `PROPLET_CLIENT_KEY`            | The client key (secret) used as the MQTT password.                                                   |                        |
| `PROPLET_DESCRIPTION`           | A human-readable description of this proplet instance.                                               | `""` (empty)           |
| `PROPLET_TAGS`                  | Comma-separated list of tags for this proplet (e.g., `"edge,prod"`).                                 | `""` (empty)           |
| `PROPLET_LOCATION`              | Physical or logical location of this proplet (e.g., `"rack-3-shelf-2"`).                             | `""` (empty)           |
| `PROPLET_COLLECT_SYSTEM_INFO`   | When `true`, automatically collects ip, os, hostname, cpu_arch, memory, and environment metadata.    | `true`                 |
...
## Running Multiple Proplets

In a multi-proplet deployment, credentials are supplied through `config.toml` instead of individual environment variables. Each proplet process reads the section matching the `PROPLET_CONFIG_SECTION` environment variable (default: `"proplet"`). New proplets can be added to a running deployment without re-provisioning using `propeller-cli provision add-proplets`.

See [Adding More Proplets](./getting-started#adding-more-proplets-to-an-existing-deployment) in the Getting Started guide for the full walkthrough, including `config.toml` structure and Docker Compose configuration.
...
1. **Startup Notification (`create` topic):**
   ...
   The payload includes the proplet's identity and enriched metadata so the Manager can immediately register all known attributes:
...
2. **Liveliness Updates (`alive` topic):**
   ...
   The payload contains the proplet's identity and current namespace. This helps the Manager maintain an updated map of active proplets and their channels:
