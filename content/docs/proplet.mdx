...
**2. Targeting filter** — If the payload includes a `proplet_id`, the proplet checks whether it matches its own ID. If not, the command is silently dropped. If `proplet_id` is absent, the task is a broadcast and all proplets execute it.

**3. Duplicate detection** — If the task is already running, the proplet ignores the duplicate command.

**4. Binary acquisition** — The proplet obtains the WebAssembly binary via one of two paths:
