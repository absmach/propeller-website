...
See [Adding More Proplets](./getting-started#adding-more-proplets-to-an-existing-deployment) in the Getting Started guide for the full walkthrough, including `config.toml` structure, Docker Compose configuration, and the `add-proplets embed` subcommand for embedded devices.
