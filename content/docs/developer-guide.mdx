...
- [A Go compiler (Go 1.25.8)](https://go.dev/doc/install)
...
TinyGo supports Go versions 1.19 through 1.23, but the project's `go.mod` requires Go 1.25.8. This creates a compatibility issue for building WASM examples.
...
The Makefile has been updated to use Go 1.23.5 for WASM builds instead of 1.25.8. This allows `make addition`, `make compute`, and other WASM examples to compile successfully while the rest of the project uses Go 1.25.8.
...
**Solution**: The project uses Go 1.25.8 but TinyGo only supports Go 1.19-1.23. Install Go 1.23.5:
...
go version      # Should be 1.25.8 (Go 1.23.5 will be used for WASM builds)
