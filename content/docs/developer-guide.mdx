...
- [A Go compiler (Go 1.25.5)](https://go.dev/doc/install)
- [Rust](https://rustup.rs/) (required for `make proplet` and `make all`)
- [Make](https://www.gnu.org/software/make/manual/make.html)
- [Docker](https://docs.docker.com/) (required for `make start-supermq` and Docker image builds)
...
TinyGo supports Go versions 1.19 through 1.23, but the project's `go.mod` requires Go 1.25.5. This creates a compatibility issue for building WASM examples.

**Workaround**: Install Go 1.23.5 separately and it will be used automatically:

```bash
go install golang.org/dl/go1.23.5@latest
~/go/bin/go1.23.5 download
```

The Makefile has been updated to use Go 1.23.5 for WASM builds instead of 1.25.5. This allows `make addition`, `make compute`, and other WASM examples to compile successfully while the rest of the project uses Go 1.25.5.
...
The project uses [mockery](https://vektra.github.io/mockery/latest/installation/) to generate mock interfaces for testing. The `make test` and `make test-all` targets automatically run `mockery --config .mockery.yaml` to generate mocks before running tests.

**Important**: The project migrated from mockery v2 to v3. If you have an older version installed, update it:

```bash
go install github.com/vektra/mockery/v2@latest
```

For mockery v3 compatibility (recommended), the project uses `pkgname: mocks` and generates individual mock files for each interface.
...
This target runs tests for the manager service and other core packages.
...
This lints all Go code and runs `cargo fmt --all -- --check` and `cargo clippy` on the Rust proplet code.
...
To start SuperMQ (the message broker infrastructure for Propeller), use:

```bash
make start-supermq
```

This runs `docker compose -f docker/compose.yaml --env-file docker/.env up -d` and starts all required SuperMQ services.

Example output:

```bash
docker compose -f docker/compose.yaml --env-file docker/.env up -d
WARN[0000] Found orphan containers ([supermq-ws]) for this project. If you removed
or renamed this service in your compose file, you can run this command with the --remove-orphans flag to clean it up.
[+] up 30/30
 ✔ Container supermq-auth-redis      Started                                6.3s
 ✔ Container supermq-auth-db         Started                                4.9s
 ✔ Container supermq-users-db        Started                                2.5s
 ✔ Container supermq-channels-redis  Started                                4.5s
 ✔ Container supermq-domains-db      Started                                3.0s
 ✔ Container supermq-clients-redis   Started                                5.1s
 ✔ Container supermq-nats            Started                                4.4s
 ✔ Container supermq-domains-redis   Started                                2.6s
 ✔ Container supermq-spicedb-db      Started                                1.9s
 ✔ Container supermq-clients-db      Started                                5.1s
 ✔ Container supermq-channels-db     Started                                6.1s
 ✔ Container supermq-rabbitmq        Started                                5.7s
 ✔ Container supermq-jaeger          Started                                4.7s
 ✔ Container supermq-groups-db       Started                                6.6s
 ✔ Container supermq-spicedb-migrate Started                                3.8s
 ✔ Container supermq-notifications   Started                                0.0s
 ✔ Container supermq-spicedb         Started                                0.0s
 ✔ Container supermq-auth            Started                                0.0s
 ✔ Container supermq-domains         Started                                1.1s
 ✔ Container supermq-users           Started                                0.0s
 ✔ Container supermq-clients         Started                                0.0s
 ✔ Container supermq-http            Started                                0.0s
 ✔ Container supermq-coap            Started                                0.0s
 ✔ Container supermq-mqtt            Started                                0.0s
 ✔ Container propeller-manager       Started                                0.0s
 ✔ Container propeller-proxy         Started                                0.0s
 ✔ Container supermq-nginx           Started                                0.9s
```
...
After SuperMQ is running, provision Propeller resources:
...
1. Creates SuperMQ domain, channel, and user credentials
2. Generates a `config.toml` file with all necessary configuration
3. Sets up clients for manager, proplet, and proxy services

The generated `config.toml` contains SuperMQ credentials (domain_id, channel_id, client_id, client_key) that are required for all services to communicate.
...
Then uncomment the volumes sections in `docker/compose.yaml` to mount the config file into containers. This allows you to use a single `config.toml` instead of setting environment variables.

### Configuring Docker Containers with config.toml

To configure Docker containers to use your generated `config.toml`, update the `docker/compose.yaml` file to uncomment the volumes sections for manager, proplet, and proxy services.
...
After uncommenting these lines, restart SuperMQ:
...
**Prerequisites**: SuperMQ must still be running (via `make start-supermq`), and you must have a valid `config.toml` in your working directory.
...
### TinyGo WASM Build Failures

**Problem**: Building WASM examples with `make addition` fails with:

```bash
requires go version 1.19 through 1.23, got go1.25
```

**Solution**: The project uses Go 1.25.5 but TinyGo only supports Go 1.19-1.23. Install Go 1.23.5:
...
The Makefile has been updated to automatically use Go 1.23.5 for WASM builds.

### Installation Permission Denied

**Problem**: `make install` fails with:

```bash
cp: cannot create regular file '/propeller-*': Permission denied
```

**Solution**: The `$GOBIN` environment variable must be set before running `make install`:
...
Without `$GOBIN`, the install target tries to write binaries to the system root directory, which requires elevated permissions.

### Mockery Configuration Issues

**Problem**: Tests fail with mockery errors or undefined mock functions like `NewPubSub`.

**Solution**: The project has been migrated to mockery v3. Ensure you have the latest version:

```bash
go install github.com/vektra/mockery/v2@latest
```

The `.mockery.yaml` configuration file has been updated for mockery v3 compatibility. If you have custom mockery configs, refer to the [mockery v3 migration guide](https://vektra.github.io/mockery/latest/migration/).

### WASM Not Compiling

**Problem**: WASM examples don't compile after upgrading tools.

**Causes**:
1. TinyGo version incompatible with Go version - see "TinyGo WASM Build Failures" above
2. Missing wasip2 support - ensure you have TinyGo 0.34.0 or later with wasip2 target

**Verify setup**:

```bash
tinygo version  # Should be 0.34.0 or later
go version      # Should be 1.25.5 (Go 1.23.5 will be used for WASM builds)
wasmtime --version  # Should be 42.0.1 or later
```

### SuperMQ Container Startup Issues

**Problem**: Some SuperMQ containers keep restarting or failing to start.
...
Common issues:
- Port conflicts: Ensure ports in `docker/.env` are available
- Disk space: Verify sufficient disk space for database containers
- Memory: SuperMQ requires 4GB+ RAM to start all services

To clean up and restart:

```bash
make stop-supermq
docker compose -f docker/compose.yaml down -v  # Remove volumes
make start-supermq
```
