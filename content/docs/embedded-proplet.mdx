...
Before building the embedded proplet, complete the environment setup in [Deploying WAMR on Zephyr](/docs/zephyr):

1. [Set Up Zephyr Development Environment](/docs/zephyr#1-set-up-zephyr-development-environment) - Install dependencies, west, and Zephyr SDK
2. [Install ESP-IDF](/docs/zephyr#2-install-esp-idf-on-your-development-machine) - Required for ESP32-S3 flashing
3. [Test Zephyr's Integration with ESP-IDF](/docs/zephyr#3-test-zephyrs-integration-with-esp-idf) - Verify your toolchain works

After completing those steps, ensure your environment is active:
...
### Step 1: Build the Embedded Proplet

From the `embed-proplet/` directory in the Propeller repository:
...
For general Zephyr issues (permissions, `west` not found, board qualifiers, serial port conflicts), see [Potential Pitfalls and Solutions](/docs/zephyr#potential-pitfalls-and-solutions). This section covers WAMR-specific issues.
...
| Topic | Description |
| ----- | ----------- |
| [Deploying WAMR on Zephyr](/docs/zephyr) | Set up Zephyr, ESP-IDF, and WAMR development environment |
| [Proplet](/docs/proplet) | Full-featured Rust Proplet for Linux/containerized environments |
| [HAL](/docs/hal) | Hardware Abstraction Layer for different runtime environments |
| [Examples](/docs/examples) | WASM module examples including the `addition.wasm` test module |
