...
- The Propeller stack running (Manager + MQTT broker) — follow the [Getting Started](/docs/getting-started) guide
...
The `wasm_handler` decodes the base64 module and loads it into WAMR. See the [Addition (Embedded Proplet)](/docs/examples/addition-embedded) example for a concrete walkthrough.
...
WAMR on Zephyr has `WAMR_BUILD_LIBC_WASI` disabled (`0` in `CMakeLists.txt`), meaning all WASI interfaces are unavailable — no file system access, no network sockets, no environment variables. This includes both WASI P1 and WASI P2. If you are used to writing WASI modules for the standard [Proplet](/docs/proplet), you will need to write modules differently for the Embedded Proplet.
...
- **Inputs**: Each element of the task's `inputs` array is passed as an i32 argument to `main`. Maximum 16 inputs (`MAX_INPUTS` in `mqtt_client.c`); additional inputs are silently truncated
...
**Inline WASM size limit**: When delivering a module via the `file` field in the start command, the base64-encoded string is capped at `MAX_BASE64_LEN = 1024` bytes (`mqtt_client.c`), which corresponds to approximately 750 bytes of decoded WASM binary. Modules larger than this must be delivered via `image_url` using the registry pull path.
...
See the [Addition (Embedded Proplet)](/docs/examples/addition-embedded) example for a complete walkthrough of building the firmware, deploying a WASM task, and reading the device logs.
...
| [Addition Example](/docs/examples/addition-embedded) | End-to-end addition example on the Embedded Proplet |
