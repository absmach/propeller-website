...
- **Inline in the start command**: For small modules, the base64-encoded WASM is embedded directly in the `control/manager/start` payload
...
1. **Receive**: The Manager publishes to `m/<domain_id>/c/<channel_id>/control/manager/start` with the task ID, name, base64-encoded WASM module, and input values
...
- **Inputs**: Each element of the task's `inputs` array is passed as an i32 argument to `main`
