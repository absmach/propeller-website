...
The embedded proplet supports runtime credential updates without reflashing. All six credentials — WiFi SSID, WiFi password, SuperMQ `proplet_id`, `client_key`, `domain_id`, and `channel_id` — are stored in NVS (Non-Volatile Storage) flash and loaded at boot. If no NVS value exists for a field, the compile-time default is used.

### Kconfig requirements

The following options must be enabled in `prj.conf`:
...
The `storage_partition` required by the Settings subsystem is already defined by the ESP32-S3 board support files at address `0x3b0000` (192 KB). No DTS overlay is needed.

### Credential fields

| Field        | Description                                             |
|--------------|---------------------------------------------------------|
| `wifi_ssid`  | SSID of the WiFi network to join                        |
| `wifi_psk`   | WiFi password                                           |
| `proplet_id` | SuperMQ client ID (used as MQTT client ID and username) |
| `client_key` | SuperMQ client secret (used as MQTT password)           |
| `domain_id`  | SuperMQ domain ID                                       |
| `channel_id` | SuperMQ channel ID                                      |

### Boot sequence

At startup, `credentials_load()` is called with compile-time defaults:

```c
static const struct proplet_credentials defaults = {
    .wifi_ssid  = CONFIG_WIFI_SSID,
    .wifi_psk   = CONFIG_WIFI_PSK,
    .proplet_id = CONFIG_PROPLET_ID,
    .client_key = CONFIG_CLIENT_KEY,
    .domain_id  = CONFIG_DOMAIN_ID,
    .channel_id = CONFIG_CHANNEL_ID,
};
credentials_load(&creds, &defaults);
```

The function initialises the Settings subsystem and loads any values previously saved to NVS, overriding the defaults field by field. The final credential struct is then used for WiFi connection and the MQTT client connection.

### Shell commands

While the device is running, connect to its serial port at 115200 baud. The Zephyr shell prompt (`uart:~$`) accepts two commands:

**Show current credentials:**

```
uart:~$ proplet creds show
wifi_ssid  : MyNetwork
wifi_psk   : mypassword
proplet_id : <your-proplet-client-id>
client_key : <your-proplet-client-key>
...
```
uart:~$ proplet creds set <field> <value>
```

For example:

```
uart:~$ proplet creds set wifi_ssid MyNetwork
Saved wifi_ssid. Reboot to apply.

uart:~$ proplet creds set client_key <your-proplet-client-key>
Saved client_key. Reboot to apply.
```

The new value is written to NVS immediately and takes effect on the next boot. Changes do not affect the running session.

### Provisioning from the CLI

The `propeller-cli provision add-proplets embed` command automates the full provisioning flow. It creates a new SuperMQ client, connects it to the existing channel, and pushes all six credentials to the device over serial in one step:

```bash
propeller-cli provision add-proplets embed -f docker/config.toml
```

The command reads `domain_id` and `channel_id` from the existing `config.toml`, so the embedded proplet joins the same channel as all other proplets in the deployment. See [Getting Started — Embedded proplets](./getting-started#embedded-proplets) for the full walkthrough.

### MQTT authentication

The embedded proplet authenticates to the MQTT broker using the SuperMQ client credentials:

| MQTT field | Value         |
|------------|---------------|
| Client ID  | `proplet_id`  |
| Username   | `proplet_id`  |
| Password   | `client_key`  |

If `client_key` is empty, the password field is omitted from the CONNECT packet.
