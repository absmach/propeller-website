...
:::warning Common Issue: Port 1883 Conflict
...
**Expected output:**
```
[+] Building 45.2s (52/52) FINISHED
 => [manager] building...
 => [proplet] building...
 => [coordinator-http] building...
...
```
...
```
[+] Running 36/36
 ✔ Network supermq-base-net         Created
 ✔ Container supermq-nats           Started
 ✔ Container supermq-rabbitmq       Started
 ✔ Container supermq-spicedb-db     Started
 ✔ Container supermq-auth-db        Started
 ✔ Container supermq-users-db       Started
 ✔ Container supermq-clients-db     Started
 ...
 ✔ Container propeller-manager      Started
 ✔ Container propeller-proplet      Started
 ✔ Container fl-demo-coordinator    Started
```
...
supermq-auth                Up 10 seconds
supermq-channels            Up 10 seconds
supermq-clients             Up 10 seconds
supermq-domains             Up 10 seconds
...
=== Creating Clients ===
✓ Created client 'manager': a1b2c3d4-...
✓ Created client 'proplet-1': bf7e3581-4dac-472c-bcd9-a83a6ca53197
✓ Created client 'proplet-2': cf3738c1-07db-41c7-8fc3-234202cf3642
✓ Created client 'proplet-3': 095d2d54-4017-4da1-b589-0809d64aa517
✓ Created client 'fl-coordinator': e5f6g7h8-...
✓ Created client 'proxy': i9j0k1l2-...
...
:::important Save the Client IDs
Note the **proplet-1**, **proplet-2**, and **proplet-3** client IDs. These UUIDs are needed when triggering FL experiments. The script automatically updates `docker/.env` with these values.
:::

### Verify Environment Variables Were Set

```bash
grep -E "PROPLET.*CLIENT_ID|DOMAIN_ID|CHANNEL_ID" docker/.env | grep -v '=""'
```
...
DOMAIN_ID="4e484e5b-0498-4453-adf8-a6fa6a37454b"
CHANNEL_ID="c1578c6c-598e-4dbe-9a8b-5d2acd623679"
```
...
:::note Creating Initial Model Manually
...
The proxy service fetches WASM binaries from a container registry. We'll use GitHub Container Registry (GHCR) which requires authentication.

:::warning Why GHCR Instead of Local Registry?
While a local registry is simpler to set up, the proxy service attempts to connect via **HTTPS**. Local registries typically use **HTTP**, causing connection failures:
```
failed to fetch container for task: error sending request
```
Using GHCR avoids this issue as it natively supports HTTPS.
:::
...
:::tip Make Package Public (Optional)
...
Update `docker/.env` with GHCR credentials so the proxy can fetch the WASM:
...
:::info Experiment Parameters Explained
...
**Symptom:** Services fail to start with port binding error.

```
Error starting userland proxy: listen tcp4 0.0.0.0:1883: bind: address already in use
```

**Solution:** Stop the conflicting service (usually mosquitto):

```bash
sudo systemctl stop mosquitto
sudo systemctl disable mosquitto  # Prevent auto-start on boot
```

### Services Fail with "undefined network supermq-base-net"
...
**Cause:** You're running the FL compose file standalone.
...
**Symptom:** Warnings about `PROPLET_CLIENT_ID` not set or empty responses.

**Solution:**
1. Run the provisioning script first:
   ```bash
   (cd examples/fl-demo && python3 provision-smq.py)
   ```
2. Recreate services with `--force-recreate`:
   ```bash
   docker compose -f docker/compose.yaml -f examples/fl-demo/compose.yaml --env-file docker/.env up -d --force-recreate
   ```

### Manager Returns 404 on /fl/experiments

**Symptom:** `curl http://localhost:7070/fl/experiments` returns 404.

**Cause:** Manager wasn't built from source (FL endpoints not included).
...
docker compose -f docker/compose.yaml -f examples/fl-demo/compose.yaml --env-file docker/.env up -d --force-recreate manager
```

### Proxy Fails to Fetch WASM from Registry

**Symptom:** Proxy logs show connection errors:
```
failed to fetch container for task: error sending request
```
...
1. **Local registry using HTTP:** The proxy attempts HTTPS connections. Local registries typically use HTTP:
   ```
   Connection refused or TLS handshake errors
   ```
   **Solution:** Use GHCR instead (which supports HTTPS natively).

2. **Missing GHCR credentials:** Check proxy configuration:
   ```bash
   grep -E "PROXY_(AUTHENTICATE|REGISTRY)" docker/.env
   ```
   Ensure `PROXY_AUTHENTICATE=true` and credentials are set.

3. **Wrong GHCR URL format:** The URL should be just `ghcr.io`, not `https://ghcr.io`:
   ```bash
   PROXY_REGISTRY_URL="ghcr.io"  # Correct
   PROXY_REGISTRY_URL="https://ghcr.io"  # Wrong
   ```

4. **Recreate proxy after updating credentials:**
   ```bash
   docker compose -f docker/compose.yaml -f examples/fl-demo/compose.yaml --env-file docker/.env up -d --force-recreate proxy
   ```
...
**Symptom:** Coordinator logs show:
```
WARN Round timeout exceeded round_id=r-... timeout_s=60 updates=0
```

**Possible causes:**

1. **Proplets not receiving tasks:** Check manager logs:
   ```bash
   docker logs propeller-manager 2>&1 | grep -E "launched task|skipping participant"
   ```
   If you see "skipping participant: proplet not alive", the proplet isn't connected to MQTT.

2. **WASM fetch failure:** Check proxy logs:
   ```bash
   docker logs propeller-proxy 2>&1 | tail -20
   ```
   Look for GHCR authentication or network errors.

3. **Proplet execution failure:** Check proplet logs:
   ```bash
   docker logs propeller-proplet 2>&1 | tail -30
   ```
   Look for WASM runtime errors or dataset fetch failures.

**Solution:** Ensure credentials are correct and recreate all services:
```bash
docker compose -f docker/compose.yaml -f examples/fl-demo/compose.yaml --env-file docker/.env up -d \
  --force-recreate manager proplet proplet-2 proplet-3 proxy coordinator-http
```

### Weights Remain Zero After Training

**Symptom:** Model version 1 exists but weights are still `[0, 0, 0]`.

**Possible causes:**

1. **Empty datasets:** Verify datasets exist:
   ```bash
   curl -s http://localhost:8083/datasets/$PROPLET_CLIENT_ID | jq '.size'
   ```
   Should return `64`.

2. **Training client error:** Check proplet logs for WASM execution errors:
   ```bash
   docker logs propeller-proplet 2>&1 | grep -E "error|Error|failed|Failed"
   ```

3. **Rebuild WASM:** If you modified the FL client code, rebuild:
   ```bash
   cd examples/fl-demo/client-wasm
   GOTOOLCHAIN=go1.25.5 GOOS=wasip2 GOARCH=wasm go build -o fl-client.wasm fl-client.go
   cd ../../..
...
**Symptom:** Manager logs show:
```
Configure experiment failed ... error":"invalid data type
```

**Cause:** Incorrect JSON format in the experiment request.

**Solution:** The `/fl/experiments` endpoint expects the configuration directly, NOT wrapped in a `config` object:

```bash
# WRONG ❌
curl -X POST http://localhost:7070/fl/experiments \
  -d '{"config": {"experiment_id": "...", ...}}'

# CORRECT ✅
curl -X POST http://localhost:7070/fl/experiments \
  -d '{"experiment_id": "...", "round_id": "...", ...}'
```

### Debugging Tips

1. **Check all service logs at once:**
   ```bash
   docker compose -f docker/compose.yaml -f examples/fl-demo/compose.yaml --env-file docker/.env logs -f --tail 50
   ```

2. **Check specific service logs:**
   ```bash
   docker logs propeller-manager --tail 50
   docker logs propeller-proplet --tail 50
   docker logs propeller-proxy --tail 50
   docker logs fl-demo-coordinator --tail 50
   ```

3. **Verify MQTT connectivity:**
   ```bash
   docker logs propeller-proplet 2>&1 | grep -E "MQTT|connected|subscribe"
   ```
...
You've successfully completed an end-to-end federated learning demo:

1. ✅ Built and started 36 containerized services (SuperMQ + Propeller + FL)
2. ✅ Provisioned SuperMQ resources (domain, clients, channel)
3. ✅ Built WASM training client and pushed to GHCR
4. ✅ Configured proxy for authenticated GHCR access
5. ✅ Triggered FL experiment via manager API
6. ✅ Proplets trained locally on 64 samples each
7. ✅ Coordinator collected updates and triggered aggregation
8. ✅ FedAvg produced a trained global model

**Key privacy guarantee:** The raw training data never left the proplets — only weight updates were shared and aggregated.
