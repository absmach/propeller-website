...
The [Proxy service](/docs/proxy) fetches WASM binaries from OCI-compliant container registries and delivers them to proplets. In this example, we use GitHub Container Registry (GHCR) which provides native HTTPS support.
...
```
Using GHCR avoids this issue. For local registry setup with HTTPS, see the [Proxy documentation](/docs/proxy#setting-up-a-local-registry-for-development).
...
Update `docker/.env` with GHCR credentials so the Proxy can fetch the WASM binary. For full configuration options, see the [Proxy documentation](/docs/proxy#setting-up-authentication).
