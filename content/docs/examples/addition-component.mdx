...
Create a task using the `function_name` field to specify the exported component function. The `inputs` array contains the arguments that will be WAVE-encoded and passed to the function.

The `function_name` must exactly match the exported function name in the WIT interface. If they don't match, the proplet looks for a non-existent export and the task fails.
