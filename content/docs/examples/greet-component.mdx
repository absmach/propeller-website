---
title: Greet (WASM Component)
description: String input example using the WASM Component Model — pass a name, get a greeting back
---

This example demonstrates passing a **string input** to a WASM Component Model export via propeller. The component takes a name and returns a personalised greeting.

## Source Code

The source code is available in the [examples/greet-component](https://github.com/absmach/propeller/blob/main/examples/greet-component) directory.

The WIT interface defines a single exported function:

<CodeFromSource
  src="https://raw.githubusercontent.com/absmach/propeller/refs/heads/main/examples/greet-component/wit/world.wit"
  lang="wit"
/>

The Rust implementation:

<CodeFromSource
  src="https://raw.githubusercontent.com/absmach/propeller/refs/heads/main/examples/greet-component/src/lib.rs"
  lang="rust"
/>

## Build the Component

You need the following tools installed:

- [Rust](https://rustup.rs/) with the `wasm32-wasip2` target: `rustup target add wasm32-wasip2`

```bash
cd propeller/examples/greet-component
cargo build --target wasm32-wasip2 --release
```

Your output should look like this:

```bash
   Compiling greet-component v0.1.0
    Finished `release` profile [optimized] target(s) in 1m 32s
```

## Invoking Directly with wasmtime

You can test the component binary directly without propeller:

```bash
wasmtime run --invoke 'greet("John Doe")' target/wasm32-wasip2/release/greet_component.wasm
```

Your output should look like this:

```bash
"Hello, John Doe!"
```

## Create Task

String inputs are passed as [WAVE](https://github.com/bytecodealliance/wasm-tools/tree/main/crates/wasm-wave)-encoded values (WebAssembly Value Encoding). This means there are two layers of quoting:

1. **Outer quotes** — JSON string syntax (every element in the `inputs` array is a JSON string)
2. **Inner escaped quotes `\"`** — WAVE format, which requires explicit quotes to identify a value as a string (as opposed to an identifier, tuple, or enum variant)

For example, the JSON value `"\"John Doe\""` contains the WAVE string `"John Doe"`, which is then passed to the WASM function.

```bash
curl -X POST "http://localhost:7070/tasks" \
  -H "Content-Type: application/json" \
  -d '{"name": "greet", "inputs": ["\"John Doe\""]}'
```

Your output should look like this:

```json
{
    "id": "6381afd0-cefd-434e-85d2-473576a81432",
    "name": "greet",
    "kind": "standard",
    "state": 0,
    "cli_args": null,
    "inputs": [
        "\"John Doe\""
    ],
    "daemon": false,
    "encrypted": false,
    "start_time": "0001-01-01T00:00:00Z",
    "finish_time": "0001-01-01T00:00:00Z",
    "created_at": "2026-03-26T10:22:23.697115725Z",
    "updated_at": "0001-01-01T00:00:00Z",
    "next_run": "0001-01-01T00:00:00Z",
    "priority": 50
}
```

## Upload Wasm

```bash
curl -X PUT "http://localhost:7070/tasks/6381afd0-cefd-434e-85d2-473576a81432/upload" \
  -F "file=@target/wasm32-wasip2/release/greet_component.wasm"
```

Your output should look like this:

```json
{
    "id": "6381afd0-cefd-434e-85d2-473576a81432",
    "name": "greet",
    "kind": "standard",
    "state": 0,
    "file": "...<redacted>...",
    "cli_args": null,
    "inputs": [
        "\"John Doe\""
    ],
    "daemon": false,
    "encrypted": false,
    "start_time": "0001-01-01T00:00:00Z",
    "finish_time": "0001-01-01T00:00:00Z",
    "created_at": "2026-03-26T10:22:23.697115725Z",
    "updated_at": "2026-03-26T10:22:27.341182514Z",
    "next_run": "0001-01-01T00:00:00Z",
    "priority": 50
}
```

## Start Task

```bash
curl -X POST "http://localhost:7070/tasks/6381afd0-cefd-434e-85d2-473576a81432/start"
```

Your output should look like this:

```json
{ "started": true }
```

The proplet logs will show the component export being invoked:

```bash
2026-03-26T10:22:37.451615Z  INFO Received start command for task: 6381afd0-cefd-434e-85d2-473576a81432
2026-03-26T10:22:37.457237Z  INFO Decoded wasm binary, size: 72591 bytes
2026-03-26T10:22:37.459183Z  INFO Executing task 6381afd0-cefd-434e-85d2-473576a81432 in spawned task
2026-03-26T10:22:37.462876Z  INFO Starting Wasmtime runtime app: task_id=6381afd0-cefd-434e-85d2-473576a81432, function=greet, daemon=false, wasm_size=72591, is_component=true, is_proxy=false
2026-03-26T10:22:37.463406Z  INFO Compiling WASM component for custom export 'greet' for task: 6381afd0-cefd-434e-85d2-473576a81432
2026-03-26T10:22:37.858028Z  INFO Export 'greet' for task 6381afd0-cefd-434e-85d2-473576a81432 completed, result: "Hello, John Doe!"
2026-03-26T10:22:37.936202Z  INFO Task 6381afd0-cefd-434e-85d2-473576a81432 completed successfully. Result: "Hello, John Doe!"
2026-03-26T10:22:37.936288Z  INFO Publishing result for task 6381afd0-cefd-434e-85d2-473576a81432
2026-03-26T10:22:37.936324Z  INFO Successfully published result for task 6381afd0-cefd-434e-85d2-473576a81432
```

## Get Results

```bash
curl -X GET "http://localhost:7070/tasks/6381afd0-cefd-434e-85d2-473576a81432"
```

Your output should look like this:

```json
{
    "id": "6381afd0-cefd-434e-85d2-473576a81432",
    "name": "greet",
    "kind": "standard",
    "state": 3,
    "cli_args": null,
    "inputs": [
        "\"John Doe\""
    ],
    "daemon": false,
    "encrypted": false,
    "proplet_id": "fecdb9c3-be8c-458b-a188-f974a2aa93c7",
    "results": "\"Hello, John Doe!\"",
    "start_time": "2026-03-26T10:22:37.454060334Z",
    "finish_time": "2026-03-26T10:22:37.971575688Z",
    "created_at": "2026-03-26T10:22:23.697115725Z",
    "updated_at": "2026-03-26T10:22:37.971575412Z",
    "next_run": "0001-01-01T00:00:00Z",
    "priority": 50
}
```

State `3` means the task completed successfully. The `results` field shows `"\"Hello, John Doe!\""` — the return value of `greet("John Doe")`.

## WAVE Encoding for String Inputs

WAVE (WebAssembly Value Encoding) is used to pass typed values to component exports. For strings, the value must be quoted within the JSON string:

| Wasm type | JSON input value | Decoded WAVE value |
|---|---|---|
| `string` | `"\"John Doe\""` | `"John Doe"` |
| `s32` | `"42"` | `42` |
| `f32` | `"3.14"` | `3.14` |
| `bool` | `"true"` | `true` |
