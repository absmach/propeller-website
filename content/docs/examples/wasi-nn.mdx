---
title: WASI NN
description: Run neural network inference on Propeller using WASI-NN with OpenVINO
---

This example demonstrates running machine learning inference using the WASI-NN API with an OpenVINO backend on Propeller. You'll deploy a neural network model to a proplet and execute inference through the platform.
...
1. Set up the Propeller platform with WASI-NN support
2. Prepare an OpenVINO model for inference
3. Build the WASI-NN example to WebAssembly
4. Deploy and execute inference through the Manager API
...
- Basic familiarity with the [Getting Started](/docs/getting-started) guide
...
- Shape: 1×3×224×224 (batch=1, channels=3 RGB, height=224, width=224)
- Data type: float32
- Layout: NCHW (batch, channel, height, width)
- Size: 1 × 3 × 224 × 224 × 4 bytes = **602,112 bytes**
...
Generate fresh credentials for all Propeller components (manager, proplet, proxy):
...
`"status": "pass"` indicates the manager is healthy and ready to accept requests on port 7070.
...
- `state: 0` — Task state: 0=created, 1=scheduled, 2=running, 3=completed, 4=failed
...
- `state: 0` — Still in "created" state; upload doesn't change the state.
...
| Field | Value | Meaning |
|-------|-------|---------|
| `state` | `3` | Task completed successfully (0=created, 1=scheduled, 2=running, 3=completed, 4=failed) |
...
### No Proplets Available

If `curl http://localhost:7070/proplets` returns an empty list or `{"proplets": []}`:

1. **Check proplet logs:** `docker logs propeller-proplet` — Look for MQTT connection errors or authentication failures.
2. **Verify MQTT connectivity:** `docker logs vernemq` — Ensure the MQTT broker is accepting connections.
3. **Ensure credentials are correct:** Compare `PROPLET_CLIENT_ID` and `PROPLET_CLIENT_KEY` in `docker/.env` with what was generated by provisioning.
4. **Restart proplet:** `docker compose -f docker/compose.yaml -f docker/compose.wasi-nn.yaml --env-file docker/.env restart proplet`
5. **Wait longer:** The proplet may need 15-20 seconds to fully register after startup.

### Task Stuck in State 0 or 1

If the task state remains 0 (created) or 1 (scheduled) after starting:

1. **Check for registered proplets:** `curl http://localhost:7070/proplets` — A task can't execute without available proplets.
2. **Verify WASM was uploaded:** Check that the `file` field in the task response is non-empty.
3. **Check manager logs:** `docker logs propeller-manager` — Look for errors dispatching the task.
4. **Check proplet logs:** `docker logs propeller-proplet` — The proplet may have received but failed to execute the task.

### Task Completes with State 4 (Failed)

If `state: 4`:

1. **Check the results field:** It may contain an error message.
2. **Verify cli_args:** Ensure `-S nn` is included to enable WASI-NN.
3. **Verify directory mapping:** The `--dir` format must be `host_path::guest_path`.
4. **Check model files:** Ensure all three files exist and are readable in `fixture/`.

### WASI-NN Runtime Errors

If you see errors like "wasi-nn not available" or "failed to load graph":

1. **Ensure correct image tag:** Verify `PROPLET_IMAGE_TAG="wasi-nn"` in docker/.env.
2. **Verify model format:** OpenVINO IR requires both `.xml` (graph) and `.bin` (weights) files.
3. **Check file permissions:** Model files must be readable by the container.
4. **Inspect container:** `docker exec -it propeller-proplet ls -la /home/proplet/fixture/` to verify files are mounted.

### Model Loading Errors

If the model fails to load or inference produces errors:

1. **Verify OpenVINO IR format:** The model must be exported in IR format (not ONNX, TensorFlow, or PyTorch format).
2. **Check model compatibility:** OpenVINO version in the container must support your model's opset version.
3. **Verify tensor dimensions:** Input tensor must match the model's expected input shape exactly.
4. **Check file sizes:** `tensor.bgr` should be 602,112 bytes for a 1×3×224×224 float32 tensor.

---

## Next Steps

- Explore the [WASI-NN documentation](/docs/wasi-nn) for advanced configuration options
- Try using a production model like MobileNetV2 or ResNet for real image classification
- Learn about [Task Scheduling](/docs/task-scheduling) for recurring inference jobs
- Set up [Monitoring](/docs/monitoring) for production deployments
- Explore [TEE (Trusted Execution Environment)](/docs/examples/tee) for confidential inference
