...
For a comprehensive guide on TEE concepts, Trustee setup, and encrypted workload configuration, see the [Encrypted Workloads](/docs/tee) documentation.

For this example we need to enable TEE mode on the proplet by setting the `PROPLET_TEE_ENABLED` environment variable and configuring the `KBS_URL` (Key Broker Service URL) for key retrieval.

```bash
export PROPLET_TEE_ENABLED=true
export KBS_URL="http://kbs.example.com:8082"
```
