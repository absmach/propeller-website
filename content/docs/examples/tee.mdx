---
title: TEE
description: TEE example
---

This is a Trusted Execution Environment (TEE) example. It runs the addition example inside a secure enclave, protecting code and data from the host OS. For this example we need to enable TEE mode on the proplet by setting the `PROPLET_TEE_ENABLED` environment variable.

```bash
export PROPLET_TEE_ENABLED=true
export PROPLET_KBS_URI="http://kbs.example.com:8080"
```
